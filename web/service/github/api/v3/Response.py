...
class Response(web.http.Response.Response):
    ...
    def Get(self, r, sleep_time=2, is_show=True):
        ...
        """
        if None is self.Headers.ContentType.MimeType:
#        if None is self.Headers.ContentType.mime_type:
            return None
        elif 'json' == self.Headers.ContentType.MimeType.SubType.Suffix:
#        elif 'json' == self.Headers.ContentType.suffix:
            return r.json()
        elif self.__re_content_type_raw.match(self.Headers.ContentType.MimeType.String):
